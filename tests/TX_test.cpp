...
int main(int argc, char* argv[]){
    ...
    std::vector<int16_t> barker_code = TX.overhead_encoder_.generate_barker_code(barker_code_size);

    /*append sync sequence*/
    std::vector<int16_t> overhead_bits = TX.overhead_encoder_.add_barker_to_message(bits, barker_code);

    /*bits -> QAM symbols*/
    std::vector<std::complex<double>> symbols = TX.modulator_.QAM_modulation(4, overhead_bits);
