...
std::vector<std::complex<double>> Receiver::QAM16_quantizater(std::vector<std::complex<double>> symbols){
    std::vector<std::complex<double>> true_constellation_point(16);

    std::vector<std::complex<double>> quantizate_symbols(symbols.size());

    for(const auto& item : QAM16_demapper_table){
